...
I decided whether a bug was really fixed if it worked correctly across several rounds of manual testing (playing the game). Then, I generated pytest code with Claude and ran the pytest. If the tests all passed, then I decided that the bug was really fixed.
...
Yes, Claude helped me design pytests for the corrections made the game code. I asked Claude to generate some pytests for the modifications made to the files. The tests were run and I read the logs to see if all of the tests passed or if there were some failures. Claude helped me to understand both the tests code and the logs by explaining what was being tested and what failed.
